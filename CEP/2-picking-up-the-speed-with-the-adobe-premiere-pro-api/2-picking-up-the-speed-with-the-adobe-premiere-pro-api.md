# 2 Picking Up The Speed with the Adobe Premiere Pro API
...
As expected, the value of `a` that is printed out is `2`. Maybe we want to make our printout a bit more clear by adding the text: "The value of the variable 'a' is `<INSEWRT VALUE HERE>`". To do this we can do what is called string concatenation in programming, a.k.a. putting strings together. We do this using the `+` operator and it works as you'd expect, `"hallo" + "there"` = `"hallothere"`. Well, sort of. We'll come back to that, for now we'll just do `"The value of 'a' is " + a`. 
...
As you can see the type of `a` is `string` and the type of `b` is number. And this is where our bug creeps in due to a misunderstanding between how we think JavaScript will add `"2"+3`. We think that it will treat both variables as numbers and sum `2` and `3` to give `5`. However, the default JavaScript action when summing two variables, one of type string and one of type number is to covert the number to string and then do normal string concatenation. 

Why this silly behavior you might ask? Well, it's not really silly, a choice had to be made, either convert the number to string, or the string to number, or throw an error. However JavaScript, and by extent, Adobe's ExtendScript are very lenient programming languages. Because JavaScript is such a bastardised language that is used in so many different ways on the web, usually it doens't throw errors, and even if it does it still continues on, making numerious assumptions based on the rules laid out in the ECMA standards. 

Don't worry, we won't go into all of the JavaScript ECMA standards. That's the problem, nobody can. So you learn by failing, and bugs are eneviatlble because not only do we know all the unkowns about the language we are using, we defintely don't know all the unknowns about the programs we are writing. For example, we wouldn't expect that somebody would try and insert the entire Sharespear body of work into our Premiere Pro Adobe ExtendScript Extension, but doesn't mean that somebody won't try, and them bugs emerge. 

Right, a few chapters later and we've made peace that bugs are envitable. So what can we do about their excistance in our code? Well.. We can put checks and balances in place to ensure that only what we expect is allowed to hit our code, otherwise we tell the user what he should do. Easier said than done, but who said anything was easy right? We choose to go to the moon, not because it is easy, but because it is difficult. 
...
Here we add an `if` statement to check the types of `a` and `b`. We use the `&&` the say "and", in other words, the first _and_ second satement need to hold true before we climb into the code defined between the first set of curly braces, `{}`. We then declare what should happen if our `if` condition isn't met, the `else` condition. In our case we'll assign the text: "'a' and 'b' both need to be numbers" to the variable `c`. Why do this? It comes with time whem programming, but you want to keep your code as clean as possible, so reusing the `c` variable name, allows be to only write `console.log(c)` _once_.  As opposed to:
...
Suble difference. However, cleaner code, is more maintanable code, is more sustainable code, is code with less _bugs_. To wrap up the JavaScript lesson, I'd like to _refactor_ the above code however, to illustrate that variable names are your choice, and you should make good choices. Or try at leats, variable names like `a`, `b` and `c` is not clear. You might think you know what your code is doing. But I can garentee you, you coming back in 2 months won't. Some better variable for our example is shown below, I also expand our else condition to tell the user exaclty _which_ number is in the wrong format. 
...
VS Code has all the standard debugging features you'd expect from an IDE, like breakpoint support, a variable inspector and being able to watch variables. If you've never heard these terms before, fret not, stick around, we unpack them later. As I was saying, VS Code has all te standard debugger features

for various language and by installing a debugger extensino for each language allows you debug all your applications using the same debugger. 
